    ...
    while (address < EEPROM_SIZE - ALARM_SIZE) {
       ...
       // Serial.print("Restoring on: ");
        ...
        if (Alarm::is_alarm(address)) {
            ...
            address += ALARM_SIZE;
            Serial.print("Active: ");
            Serial.println(alarms[counter]->get_active() ? "true" : "false");
            Serial.print("State: ");
            Serial.println(alarms[counter]->m_state);
            Serial.print("Time: ");
            Serial.println(alarms[counter]->start_time);
            counter++;
        } else{
